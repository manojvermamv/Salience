...
handoff and owns a distinct immutable publisher request, idempotent plan/attempt,
...
account/connection identity and a fresh policy/approval/budget decision are never
inherited from creative work.
...
edge-only in an injected session store; the safe session identity is the only
