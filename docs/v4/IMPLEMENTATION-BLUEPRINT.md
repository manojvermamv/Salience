...
All phases are **planned**, including tasks that reuse existing components. Requirement rows below specify the acceptance assertion for each slice. Test paths named in this section are future suites unless explicitly identified as existing. No checkbox is complete merely because this blueprint exists.
...
**Dependencies:** RG0. **Delivers:** V4 increment 1 and common outbox/recovery envelope for all following phases.
...
- [ ] Add durable defer/review/retry/reconciliation waits with caps and owners, terminal archive commits, and failure records. Replay histories across worker upgrades and Continue-As-New without changing logical IDs. Add API/CLI/SDK parity tests for new commands.
...
- **P1 re-admission:** persist immutable admission dispositions with a mutable intent eligibility revision. Deferred/awaiting-review intent has no cycle until admitted. A timer or bound approved review advances the revision and retries the same intent with a unique admission key; a unique admitted-intent constraint permits one cycle only. Review checks current revision, expiry and cancellation. Deferral after cycle creation resumes the existing cycle; a genuinely new business attempt requires a separately authorized, coalesced successor intent. Test duplicate wakes, concurrent review/timer, stale approvals and crash on each commit boundary.
...
| R03 | §1: atomic business transition/outbox, acknowledged ordered delivery and deduplication | P1 | P0,R01 | DB-commit/start-loss and consumer-ack-loss faults recover one ordered business operation without duplicate resource reservation. | M1: shadow outbox then cut over dispatch, keep unacked rows/consumer ledger on rollback. | RG1 |
...
| R06 | §2: stable CycleIntent/cycle ID, slots and manual/event idempotency across revisions | P1 | R03,R05 | Concurrent/retried/manual/event/updated-schedule inputs for same business slot create one logical cycle; new Temporal Run ID does not create a new operation. | M1: slot uniqueness/coalescing and legacy mapping; retain mapping during rollback. | RG1 |
...
| R09 | §2: defer/abstain closes cycle, durable wake coalesces, no recursive research cycles | P1 | R07,R08 | Time-skipping tests see finite timer/event wakes, new eligible intent on later work and no schedule-plus-timer duplication; research exhausts depth/call/cost/deadline into disposition. | M1: retain wake key and closed disposition; drain original waits on rollback. | RG1 |
