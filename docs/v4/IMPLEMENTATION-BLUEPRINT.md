...
**Status as of 2026-09-24: P0 locally qualified; P1 partially implemented and ongoing; P2–P7 planned. Production RG0 and full RG1 remain HELD.** Source checkpoint: `483b905f1437c8b3291f993edbca864f7e909ce1`, containing application checkpoint `e628ccf478988b4db553983d990f43a3e3ca289a`. Documentation baseline: `8e50d68efbf2bec2aabfb2fe3226e01635de2adb`, inspected 2026-09-22. The numbered P0–P7 phases below are new V4 delivery phases, not the old Phase 1–9 labels.
...
Read this document in order, reuse the completed work in the current execution snapshot, and resume at the first unfinished P1 obligation using **Understand → Spec → Verifier → Environment → Implement → Verify → Inspect → Repair → Re-verify → Checkpoint → Repeat**. For each slice, first write the named failing acceptance test, demonstrate the failure, implement only that slice, run focused and applicable regression tests, review migration/rollback and evidence, then checkpoint. The original documentation transition changed no application behavior; subsequent authorized P0 and local P1 implementation are distinguished below. Future implementation needs no historical plan to interpret these requirements.

## Navigation

- [Current execution snapshot](#current-execution)
- [Ordered P1 resume queue](#p1-resume-queue)
...
This is the current summary; the dated baseline observations and chronological checkpoints below remain historical evidence. The 2026-09-24 refresh changes documentation only. The latest executed application qualification is **317 full non-live tests and 47 focused P1/migration tests passed on 2026-09-23**, with no failures/errors/skips; it is not a newly executed application run on the refresh date. Fourteen documentation tests, six Mermaid diagrams and the 75-obligation contract have recorded passing evidence. See [validation](validation.md) for fresh documentation checks and the [release ledger](p0-release.json) for gate status.

| Completed local work | Actual implementation and evidence | Boundary still outstanding |
| --- | --- | --- |
| P0 substrate qualification | Migrations 0013–0015; authenticated isolated control, narrow delegated identity locking, scoped credentials, immutable object inventory/readback/retention, OTel correlation and effects lockout; genuine restricted-role tests | Production issuer/TLS/rotation/DB provisioning, managed encrypted storage/restore, telemetry/alerts and deployment safety approval remain NOT RUN. |
| Corrected gates and development entry | Acyclic 75-obligation mapping, fail-closed mandatory release checks and durable qualification evidence; DG1 PASS before P1 implementation | Local development entry never releases RG0, RG1 or enables production effects. |
| P1a admission and lifecycle | Migrations 0016–0017; bounded dry-run intents/admissions/cycles/context, current scope, three distinct deferral/recovery identities and successor coalescing | No paid G0 ledger, full schedule policy, production review cases or public V4 commands. |
| P1b ordered outbox and fixture runtime | Migration 0018; transactional messages/inbox, ordered token-fenced leases, bounded retries/dead letters, receipt reconciliation, actual worker SIGKILL/restart/history replay and SDK trace correlation | No automatic dispatcher service, legacy dual-write cutover, complete API-to-provider trace path or worker-upgrade qualification. |
| Goal revision integrity | Migration 0019; atomic CAS/command receipts/audit, immutable payloads and context, stable intent/successor retries after shorter horizons and terminal state, stale-authority holds | Exact retries recover existing identity, not renewed authority. Case-bound renewal and complete goal/authority/cadence fields remain planned. |
| Explicit fixture baseline | Migration 0020; separate `goals:approve`, existing plugin registry, exact no-effect manifest/strategy/prompt pinning, immutable approval and revocation, current expiry checks | No general live-provider selection or capability evolution. Historical unapproved contexts never inherit later approval. |
| Repository controls | Required `verify` and `p0-regression` checks PASS for `483b905`; protection re-read on 2026-09-24 with strict app-bound checks, administrator enforcement and current independent PR approval | Any later commit needs its own CI. No main merge or independent GitHub PR approval is claimed. |

Application source is unchanged since `e628ccf`; the Alembic head is `0020_goal_baselines`. `GoalSpec.local.v1` still enforces dry run, zero spend, `fixture.dummy@1.0.0` and fallback denial. The cycle worker is explicitly constructed for private local fixture queues, not registered by the production worker. Acknowledged runtime delivery is not provider success, and a held inbox receipt is not approval to execute. `wake(review=True)` is an internal scoped fixture command, not the final version-bound human-review API.

**Ongoing versus blocked:** P1 is an ongoing implementation phase, not a claim that a background task is currently running. Its remaining local work is executable after DG1. External production qualifications and independent PR approval block deployment/merge as applicable; they do not falsely make unfinished local P1 code an external blocker. P2–P7 remain planned under their declared gates, with optional P6 disabled. No live publication, model/tool provider, learning, experiment or training capability is authorized by these increments.
...
**Implemented local increment P1a:** `cycles/contracts.py` and `cycles/admission.py`, migrations 0016–0017, and `tests/integration/test_v4_cycle_admission.py` implement a private dry-run-only command service with scoped current DB authority, a frozen `GoalSpec.local.v1`, goal state transitions, stable coalesced intent identities, immutable admission attempts and context, bounded local recovery, and committed strategic closures/successor lineage. PostgreSQL serializes same-goal decisions; SQL guards forbid rewriting intent/cycle/operation identity, context and closed disposition. Pre-admission wakes CAS the eligibility revision and recheck current scope/due/expiry/limits; review requires a separate current grant. Restart/failure injection proves rollback before commit and deterministic repeat after commit. P1a itself added no API, CLI, SDK, scheduler or worker path; subsequent P1b adds the isolated fixture runtime described below. Its originally fixed fixture declaration is now subject to the explicit registry-backed baseline approval added in 0020; no real provider executes. No paid allocation/reservation is created: the DTO rejects nonzero spend, non-dry mode and fallback. This is partial evidence for R05–R10/R13/R32/R35/R53, not fulfillment of their complete RG1 assertions.

**Next executable P1 increments:** (c) complete remaining goal/context/authority fields and cadence policy, reusing the completed atomic revision commands and explicit fixture baseline, and integrate G0 allocations with the existing budget ledger (and P2 G1 transfer contract); (d) version-bound case/review/resume/archive and serialized stop/permit control, API/CLI/SDK parity; (e) extend the completed P1b outbox with automatic dispatch, full API-to-provider trace integration and legacy start/signal cutover, then qualify worker upgrades/Continue-As-New, schedule watermarks, rollback and independent review. Reuse the already verified atomic outbox/inbox, ordered delivery, restart/ack-loss and local trace tests rather than rebuilding them. P1a's `wake(review=True)` is an internal testable scoped command, not the final case/artifact-bound human-review API; do not expose it as that API. Complete every unchecked acceptance below before RG1. Existing legacy fixture dual-write paths are unchanged and may not be promoted or converted to live execution by this slice.
...
**Implemented local goal-revision subincrement:** `CycleAdmission.revise_goal` requires current `goals:write`, an expected revision, bounded command key and reason. Migration 0019 stores immutable exact-input command receipts and guards the goal pointer/identity/state. The goal lock precedes the separate revision-payload read, so concurrent updates cannot hide a newly committed payload behind an older statement snapshot. Revision payload, pointer, receipt and audit commit together; a crash rolls all four back. An exact authorized command retry returns its original revision even after subsequent revisions, while changed input or a competing stale command fails. Existing slot retries retain the original intent across revisions, shortened horizons and terminal goal states; this lookup creates no new admission or successor authority. New intents and successor links still require an active goal and current valid time window. Stale unadmitted intents deny, and stale review wakes, recovery transitions and new fixture consumption hold. Existing frozen context is never rewritten; terminal closure remains available. SQL rejects goal identity changes, terminal reactivation and revision regression/skips/missing payloads. `tests/integration/test_v4_goal_revisions.py` and the extended migration suite cover these boundaries. This does not implement full cadence, approved baseline selection, renewed case-bound authority or paid G0/G1 accounting.

**Goal-revision rollback:** disable revision writes and new admission; preserve the command receipts, historical payloads, frozen contexts and current goal pointer. Migration 0019 refuses downgrade when revision receipts exist. Never reset the pointer to resurrect superseded authority. This local-only addition does not enable effects or satisfy full RG1.

**Implemented explicit fixture baseline:** `cycles/baselines.py` reuses the existing `PluginRegistry`/`PluginManifest` to validate the exact `fixture.dummy@1.0.0` capability contract, with no effects, secrets, cost or fallback. Separate `goals:approve` authority records the fixed fixture strategy/prompt/manifest, expiry, actor, trace and reason against an exact goal revision; normal goal/cycle grants cannot approve it. Migration 0020 stores immutable approvals and append-only revocations. Admission without a current approval returns `review_required` and creates no cycle. Approval and revocation serialize with admission and consumption on the goal row. Frozen context records the exact approval ID and planned bundle; revocation/expiry/stale revision holds subsequent consumption and recovery. An exact approval retry returns historical identity, never reverses revocation. Material revisions need new approval. Recovery additionally requires the original context's exact approval reference: approving a historical goal never retroactively authorizes its old unapproved cycle. New fixtures explicitly approve; no migration fabricates approvals. This is the local baseline subincrement of R05/R10/R13, not general registry eligibility/trials/upgrades or a production effect permit.

**Baseline migration/rollback:** apply 0020 additively with ingress/effects disabled; old goals and pending cycles with no approved frozen bundle hold rather than being backfilled. Preserve original cycle/context identity and resolve through later explicit case-bound renewal, never a fresh implicit operation. Disable baseline/admission writes for rollback; populated approvals/revocations prohibit destructive downgrade. `tests/integration/test_v4_baseline_approval.py` exercises scope separation, exact/conflicting/concurrent approval, missing/foreign/invalid/expired/revoked bundles, immutable context/history, admission/revocation race, transactional rollback and historical-context hold; the migration suite proves preservation.

<a id="p1-resume-queue"></a>
#### Ordered remaining work

These items are **planned, not implemented**. Reuse the completed slices above and the existing cost, workflow, control and registry modules. Complete each applicable P1 matrix assertion before RG1; this queue does not waive any requirement or move it into a later phase.

| Order | Remaining deliverable | Required verifier before completion | Migration/rollback constraint |
| --- | --- | --- | --- |
| 1 | Complete goal/context/authority fields and explicit overlap, catch-up, stale-slot/backfill and manual/event/scheduled coalescing policies | Extend admission/revision suites: bounded backlog, schedule-revision dedupe, current authority, immutable planned versions, no unapproved baseline or implicit trial assignment | Add compatible fields/sidecars; preserve old intents, receipts and frozen contexts. Do not reinterpret old approval. |
| 2 | Integrate G0 cycle allocations with the existing scoped budget ledger and define/test the P2 G1 transfer contract | Concurrent allocation/transfer, no double commitment, release only unallocated authority, cancellation/unknown liability retention, duplicate/late settlement and crash reconstruction | Lock applicable budget dimensions deterministically; append accounting identities. Never release dispatched or unknown liability on rollback. Nonzero production spend stays disabled. |
| 3 | Serialized stop/dispatch permits and typed case/review/resume/archive contracts | Both stop/claim orderings, current authority outage, expiry, duplicate/late review, artifact/context/version binding, original operation identity and committed archive | Append authority revisions and case/permit records; retain issued claims and immutable decisions, never reactivate revoked approval. |
| 4 | Automatic bounded outbox dispatch, terminal-delivery recovery and complete API → runtime → activity → adapter trace integration | Process failure at every handoff, dead-letter escalation, exact receipt reconciliation, secret-canary checks and no duplicate external fixture effect | Shadow first, retain ordered unconsumed messages; handoff acknowledgement is never effect completion. Do not register a production provider. |
| 5 | API/CLI/SDK parity and canonical schedule conversion | Identical authorization, idempotency, inspect/cancel/events semantics; no stage-start bypass; schedule/manual/wake coalescing | Cut over one scoped goal with a durable watermark and disable its old path; keep legacy readers for legacy jobs only. |
| 6 | Supported worker upgrades, Continue-As-New, real durable review deadlines and restart/restore qualification | Replay old/new histories, bounded history and waiting, stable cycle/context/operation IDs, timeout owner/escalation and safe rollback | Pin compatible workers until replay/drain passes; preserve canonical data through rollback. |
| 7 | Independent full RG1 review and final evidence reconciliation | All P1 matrix tests, full regressions, current-head CI/enforcement and reviewed release ledger; deployment also requires RG0 | Keep RG1 HELD while any applicable acceptance is unfinished; no automatic main merge or production enablement. |

**Bound decisions to retain:** pre-admission waits reuse the intent without a cycle; retry/reconciliation reuses the cycle; strategic defer/abstain closes it and later uses an authorized coalesced successor. G0 allocation transfers into G1, rather than double-reserving. Trial/evaluation isolation precedes P4. Mandatory versus optional capability gates, including R58, and concurrent evidence invalidation versus promotion retain the phase-specific rulings in section 6. They are specified decisions, not implemented later-phase systems.
...
| R05 | §2: versioned GoalSpec, complete scope/metric/cadence/budget/stop fields and approved baseline | P1 | P0,R01 | Local goal-revision/baseline tests prove atomic CAS, exact retries, immutable history, terminal/stale guards and explicit scoped fixture approval; full scope/cadence/budget fields and production baseline still require qualification. | M1: new revisions, no authority inferred from program rows; disable new goals on rollback. | RG1 |
| R06 | §2: stable CycleIntent/cycle ID, slots and manual/event idempotency across revisions | P1 | R03,R05 | Local admission and goal-revision tests prove same-slot concurrency, successor coalescing, retry identity across revisions/shorter horizons and rollback/restart; full manual/event/schedule revision and Temporal Run ID tests remain required before RG1. | M1: slot uniqueness/coalescing and legacy mapping; retain mapping during rollback. | RG1 |
...
| R10 | §3: authoritative immutable context resolved after admission and before affected work | P1 | R07 | Local baseline tests freeze exact approved fixture bundle/prompt/authority and hold unapproved historical contexts; full stage execution still needs committed bundle/cutoff/scope and crash-resumable context resolution. | M1: context sidecars with legacy_unknown; compatible readers keep original bundles. | RG1 |
...
| R13 | §3: current revocation constrains pinned context; revisions invalidate approvals | P1 | R10 | Local revision/baseline tests hold stale wakes and unapproved recovery/consumption, rechecking expiry/revocation without rewriting context; full version revocation and case-bound renewed effect approval remain required. | M1: append linked revisions/invalidations; do not restore revoked approval on rollback. | RG1 |
