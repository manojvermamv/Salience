...
| R05 | §2: versioned GoalSpec, complete scope/metric/cadence/budget/stop fields and approved baseline | P1 | P0,R01 | Local goal-revision tests prove atomic CAS, exact retries, immutable history and terminal/stale guards; full scope/cadence/budget fields and explicitly approved baseline still require qualification. | M1: new revisions, no authority inferred from program rows; disable new goals on rollback. | RG1 |
...
| R13 | §3: current revocation constrains pinned context; revisions invalidate approvals | P1 | R10 | Local goal-revision tests hold stale wakes/recovery/consumption without rewriting context; full version revocation and case-bound renewed effect approval remain required, preserving original assignment/operation. | M1: append linked revisions/invalidations; do not restore revoked approval on rollback. | RG1 |
